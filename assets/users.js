...
class UsersUtils {
    ...
    /**
     * Валидация ID працівника
     */
    static validateEmployeeId(employeeId) {
        if (!employeeId) return false;
        
        // Простая проверка - только цифры и буквы, длина от 3 до 20 символов
        return /^[A-Za-z0-9]{3,20}$/.test(employeeId);
    }

    /**
     * Генерация предложения пароля
     */
    static generatePassword(length = 8) {
        return ApiUtils.generatePassword(length);
    }

    /**
     * Форматирование статуса пользователя
     */
    static formatUserStatus(user) {
        if (!user) return 'Невідомо';
        
        if (!user.is_active) return 'Неактивний';
        if (!user.last_login) return 'Ніколи не входив';
        ...
        if (diffHours < 24) return 'Активний сьогодні'; 
		if (diffHours < 168) return 'Активний на тижні';
        
        return 'Давно не входив';
    }

    /**
     * Сортировка пользователей
     */
    static sortUsers(users, sortBy, direction = 'asc') {
        return users.sort((a, b) => {
            ...
            switch (sortBy) {
                ...
                case 'employee_id':
                    ...
                    break;
                case 'last_login':
                    valueA = a.last_login ? new Date(a.last_login) : new Date(0);
                    valueB = b.last_login ? new Date(b.last_login) : new Date(0);
                    break;
                case 'role':
     ...
     */
    static filterUsers(users, searchText) {
        ...
        return users.filter(user => {
            return (
                ...
                user.faculty_name?.toLowerCase().includes(search) ||
    ...
    static getUsersStats(users) {
        ...
        users.forEach(user => {
            ...
            // По факультетам
            const faculty = user.faculty_name || 'Не вказано';
            stats.byFaculty[faculty] = (stats.byFaculty[faculty] || 0) + 1;

            // Недавние входы
            if (user.last_login && new Date(user.last_login) > weekAgo) {
                stats.recentLogins++;
            }
        });
    ...
    static exportUsersToCSV(users) {
        ...
        const rows = users.map(user => [
            ...
            user.last_login ? ApiUtils.formatDateTime(user.last_login) : 'Ніколи',
            user.created_at ? ApiUtils.formatDateTime(user.created_at) : ''
        ]);
...
/**
 * Компонент для работы с ролями пользователей
 */
class UserRolesManager {
    ...
    async loadRoles() {
        try {
            const response = await window.api.getRoles();
            if (response.success) {
                this.roles = response.data;
                this.parsePermissions();
            }
        } catch (error) {
            console.error('[UserRoles] Ошибка загрузки ролей:', error);
        }
    }

    /**
     * Парсинг разрешений из ролей
     */
    parsePermissions() {
        this.roles.forEach(role => {
            if (role.permissions) {
                try {
                    this.permissions[role.name] = JSON.parse(role.permissions);
                } catch (error) {
                    console.error(`[UserRoles] Ошибка парсинга разрешений для роли ${role.name}:`, error);
                }
            }
        });
    }
     ...
     */
    hasPermission(roleName, permission, resource = null) {
        const rolePermissions = this.permissions[roleName];
        if (!rolePermissions) return false;

        if (resource && rolePermissions[resource]) {
    ...
    getRoleDescription(roleName) {
        const descriptions = {
            ...
            'dekanat': 'Доступ до користувачів та звітів факультету',
            'zaviduvach': 'Доступ до користувачів та звітів кафедри',
            'vykladach': 'Доступ до власних досягнень'
        };

        return descriptions[roleName] || 'Роль не описана';
    }
}

/**
 * Компонент для массовых операций с пользователями
 */
class BulkUsersOperations {
    constructor() {
    ...
    init() {
        ...
        this.isInitialized = true;
        console.log('[BulkOperations] Компонент массовых операций инициализирован');
    }
    ...
    setupBulkControls() {
        ...
        selectAllCell.innerHTML = `
            <input type="checkbox" id="select-all-users" title="Вибрати всіх">
        `;
        tableHeader.insertBefore(selectAllCell, tableHeader.firstChild);

        // Добавляем панель массовых операций
        const usersSection = document.getElementById('users-section');
        if (usersSection) {
            const bulkPanel = document.createElement('div');
            bulkPanel.id = 'bulk-operations-panel';
            bulkPanel.className = 'bulk-panel hidden';
            bulkPanel.innerHTML = `
                <div class="bulk-info">
                    <span id="selected-count">0</span> користувачів вибрано
                </div>
                <div class="bulk-actions">
                    <button class="btn btn-secondary" id="bulk-export">
                        <i class="fas fa-download"></i>
                        Експортувати вибраних
                    </button>
                    <button class="btn btn-danger" id="bulk-deactivate">
                        <i class="fas fa-ban"></i>
                        Деактивувати вибраних
                    </button>
                    <button class="btn btn-outline" id="bulk-cancel">
                        Скасувати вибір
                    </button>
                </div>
            `;
            
            const tableContainer = usersSection.querySelector('.users-table-container');
            if (tableContainer) {
                tableContainer.parentNode.insertBefore(bulkPanel, tableContainer);
            }
        }
    }
    ...
    setupEventListeners() {
        ...
        if (bulkExport) {
            bulkExport.addEventListener('click', this.handleBulkExport.bind(this));
        }

        if (bulkDeactivate) {
            bulkDeactivate.addEventListener('click', this.handleBulkDeactivate.bind(this));
        }

        if (bulkCancel) {
            bulkCancel.addEventListener('click', this.handleBulkCancel.bind(this));
        }

        // Обработчик изменения чекбоксов строк
    ...
    /**
     * Добавление чекбоксов к строкам пользователей
     */
    addCheckboxesToRows() {
        const userRows = document.querySelectorAll('#users-table-body tr[data-user-id]');
        
        userRows.forEach(row => {
            const userId = row.dataset.userId;
            const firstCell = row.querySelector('td');
            
            if (firstCell && !firstCell.querySelector('.user-checkbox')) {
    ...
    /**
     * Обновление панели массовых операций
     */
    updateBulkPanel() {
    ...
    handleBulkExport() {
        if (this.selectedUsers.size === 0) {
            window.toast.warning('Не вибрано жодного користувача');
            return;
        }

        const selectedUsersData = Array.from(this.selectedUsers)
            .map(userId => window.users.getUserById(userId))
            .filter(user => user);

        UsersUtils.exportUsersToCSV(selectedUsersData);
        window.toast.success(`Експортовано ${selectedUsersData.length} користувачів`);
    }
    ...
    handleBulkDeactivate() {
        ...
        window.modals.showConfirmation({
            ...
            message: `Ви впевнені, що хочете деактивувати ${this.selectedUsers.size} користувачів?`,
            type: 'warning',
            confirmText: 'Деактивувати',
            cancelText: 'Скасувати',
            onConfirm: async () => {
                await this.performBulkDeactivation();
            }
        });
    }

    /**
     * Выполнение массовой деактивации
     */
    async performBulkDeactivation() {
        ...
        for (const userId of userIds) {
            try {
                ...
                console.error(`[BulkOperations] Ошибка деактивации пользователя ${userId}:`, error);
                errorCount++;
            }
        }

        if (successCount > 0) {
            window.toast.success(`Деактивовано ${successCount} користувачів`);
            await window.users.loadUsers();
        }

        if (errorCount > 0) {
            window.toast.error(`Помилок при деактивації: ${errorCount}`);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    // Добавляем стили для массовых операций
    ...
    style.textContent = `
        ...
        .role-dekanat { background: rgba(124, 58, 237, 0.1); color: #7c3aed; }
        .role-zaviduvach { background: rgba(5, 150, 105, 0.1); color: #059669; }
        .role-vykladach { background: rgba(37, 99, 235, 0.1); color: #2563eb; }
...
window.UsersUtils = UsersUtils;
window.userRoles = new UserRolesManager();
window.bulkOperations = new BulkUsersOperations();